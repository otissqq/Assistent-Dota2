...
class HomePage(QWidget):
    ...
    def on_analyze_clicked(self):
        ...
        if not self.ally_heroes and not self.enemy_heroes:
            # Demo fallback: if the user has not loaded a screenshot at all,
            # keep the old sample draft behavior so the button still works.
            result = vision_service.recognize_draft("__no_file__", side=self.side)
            self.ally_heroes = result["ally_heroes"]
            self.enemy_heroes = result["enemy_heroes"]
            self._render_placeholder_heroes()

        # If computer vision recognized only part of the draft, fill empty
        # allied slots with AI/local recommendations before the final analysis.
        # Example: recognized 3 allies and 4 enemies -> add 2 recommended allies.
        if 0 < len(self.ally_heroes) < 5:
            missing = 5 - len(self.ally_heroes)
            picks, reason, used_ai_for_fill = gemini_service.suggest_missing_picks(
                ai_key,
                self.ally_heroes,
                self.enemy_heroes,
                missing,
            ...
            if picks:
                self.ally_heroes.extend(picks)
                self._render_placeholder_heroes()
                source = "Gemini" if used_ai_for_fill else "локальний алгоритм"
                autofill_note = (
                    f"\n\nАвтодоповнення драфту ({source}): додано "
                    f"{', '.join(picks)}. {reason}"
                )
        ...
        analysis["ai_text"] = ai_text + autofill_note
